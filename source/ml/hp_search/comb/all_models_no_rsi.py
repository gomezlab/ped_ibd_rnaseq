...
for outcome in ['B2', 'B3', 'rem', 'surg']:
    ...
    plt.xlim([0.0, 1.0])
    plt.savefig('results/roc_comb_{}_norsi.png'.format(outcome), bbox_inches='tight')
    plt.clf()
    ...
    plt.xlim([0.0, 1.0])
    plt.savefig('results/prc_comb_{}_norsi.png'.format(outcome), bbox_inches='tight')
    ...
    results.to_csv('results/results_comb_{}_norsi.csv'.format(outcome), index=False)
