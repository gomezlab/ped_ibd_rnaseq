...
clin_cols = ['sex', 'dx_age', 'initial_b', 'location', 'perianal_disease', 'fam']
...
xgb_scores = pd.DataFrame(columns=['feature_count', 'n_estimators', 'max_depth', 'auroc', 'auprc'])

fold_cols = {}
count = 0
for feat_count in [0.01, 0.05, 0.1]:
    for n_estimators in [200, 500, 1000]:
        for max_depth in [6, 12, 24]:
            ...
            for train_index, val_index in cv.split(X):
                ...
                xgb = XGBClassifier(n_estimators=n_estimators, max_depth=max_depth, tree_method='gpu_hist', use_label_encoder=False, eval_metric='logloss', n_jobs=-1)
            ...
            xgb_scores.loc[count] = [feat_count, n_estimators, max_depth, auroc, auprc]
            count += 1
            del xgb_y_true, xgb_y_pred

xgb_scores.sort_values(by='auroc', ascending=False, inplace=True)
xgb_scores.to_csv('hp_results/{}/xgb_{}.csv'.format(rsi, outcome))